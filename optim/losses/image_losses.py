    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        ...
        Args:
            ...
            target (N,*),
                same shape as input.
        """
        if len(input.shape) != len(target.shape) or len(input.shape) != 4:
            raise ValueError(f"Tensors must have 4 dimensions ({input.shape}) ({target.shape})")

        if input.shape[1] != 1 and input.shape[1] != 3:
            # Reshape input if it doesn't have 1 or 3 channels
            batch, channels, height, width = input.shape
            input = input.reshape(batch * channels, 1, height, width)
            target = target.reshape(batch * channels, 1, height, width)
        ...
        return loss_pl / len(input_features)
